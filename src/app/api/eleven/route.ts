...
// Helper untuk mengonversi buffer MP3 ke buffer WAV 16-bit PCM Mono
function convertMp3ToWav(mp3Buffer: Buffer, targetSampleRate: number): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    const tempInput = path.join(tempDir, `input_${Date.now()}_${Math.random().toString(36).substring(7)}.mp3`);
    const tempOutput = path.join(tempDir, `output_${Date.now()}_${Math.random().toString(36).substring(7)}.wav`);
    ...
    ffmpeg(tempInput)
      ...
      .audioChannels(1) // Mono (Sangat direkomendasikan untuk Edge Impulse & ESP32-S3)
  ...
  try {
    ...
    for (const voice of voices) {
      ...
      for (let take = 1; take <= generationsPerVoice; take++) {
        try {
          let apiOutputFormat = outputFormat;
          let isWavConversionNeeded = false;
          let targetSampleRate = 44100;

          if (outputFormat.startsWith("wav")) {
            isWavConversionNeeded = true;
            apiOutputFormat = "mp3_44100_128"; // Gunakan format MP3 yang didukung akun Free Tier
            targetSampleRate = outputFormat.includes("48000") ? 48000 : 44100;
          }

          const url = `https://api.elevenlabs.io/v1/text-to-speech/${voiceId}?output_format=${apiOutputFormat}`;
          ...
          if (isWavConversionNeeded) {
            try {
              finalBuffer = await convertMp3ToWav(buffer, targetSampleRate);
            } catch (convErr) {
              console.error("Gagal melakukan konversi MP3 ke WAV:", convErr);
              results.push({
                voiceId,
                name: nameWithTake,
                success: false,
                error: `Gagal mengonversi MP3 ke WAV secara lokal: ${convErr instanceof Error ? convErr.message : convErr}`,
              });
              continue;
            }
          }

          // Buat nama file aman dengan format: kata yang di generate_bahasaoveride_modelnama_tanggaldanjam
          const safeModelName = name
